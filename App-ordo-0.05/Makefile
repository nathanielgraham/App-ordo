# This Makefile is for the App::ordo extension to perl.
#
...
#     ABSTRACT => q[A utility module for the ordo script]
#     AUTHOR => [q[Nathaniel Graham <ngraham@cpan.org>]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => { ExtUtils::MakeMaker=>q[0], File::ShareDir::Install=>q[0.06] }
#     DISTNAME => q[App-ordo]
#     EXE_FILES => [q[script/ordo]]
#     LICENSE => q[perl]
#     MIN_PERL_VERSION => q[5.008004]
#     NAME => q[App::ordo]
#     PREREQ_PM => { Data::Dumper=>q[0], Date::Parse=>q[0], DateTime::TimeZone=>q[0], Email::Valid=>q[0], Exporter=>q[0], File::Basename=>q[0], File::Copy=>q[0], File::Path=>q[0], File::ShareDir=>q[0], File::Spec=>q[0], Getopt::Long=>q[0], IO::Socket::INET=>q[0], JSON=>q[0], Mojo::UserAgent=>q[0], POSIX=>q[0], Term::ANSIColor=>q[0], Term::ReadKey=>q[0], Term::ReadLine::Perl5=>q[0], Text::Table::Tiny=>q[1.02], Time::Local=>q[0], feature=>q[0], strict=>q[0], warnings=>q[0] }
#     TEST_REQUIRES => {  }
#     VERSION => q[0.05]
#     test => { TESTS=>q[] }
...
EXE_FILES = script/ordo

pure_all :: $(INST_SCRIPT)/ordo
	$(NOECHO) $(NOOP)
...
$(INST_SCRIPT)/ordo : script/ordo $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/ordo
	$(CP) script/ordo $(INST_SCRIPT)/ordo
	$(FIXIN) $(INST_SCRIPT)/ordo
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/ordo
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''A utility module for the ordo script'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '  - '\''Nathaniel Graham <ngraham@cpan.org>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: App-ordo' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   "abstract" : "A utility module for the ordo script",' >> META_new.json
	$(NOECHO) $(ECHO) '   "author" : [' >> META_new.json
	$(NOECHO) $(ECHO) '      "Nathaniel Graham <ngraham@cpan.org>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "App-ordo",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '            "Email::Valid" : "0",' >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="App-ordo" VERSION="0.05">' > App-ordo.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>A utility module for the ordo script</ABSTRACT>' >> App-ordo.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Nathaniel Graham &lt;ngraham@cpan.org&gt;</AUTHOR>' >> App-ordo.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> App-ordo.ppd
	$(NOECHO) $(ECHO) '        <PERLCORE VERSION="5,008004,0,0" />' >> App-ordo.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Data::Dumper" />' >> App-ordo.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Date::Parse" />' >> App-ordo.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="DateTime::TimeZone" />' >> App-ordo.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Email::Valid" />' >> App-ordo.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Exporter::" />' >> App-ordo.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="File::Basename" />' >> App-ordo.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="File::Copy" />' >> App-ordo.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="File::Path" />' >> App-ordo.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="File::ShareDir" />' >> App-ordo.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="File::Spec" />' >> App-ordo.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Getopt::Long" />' >> App-ordo.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="IO::Socket::INET" />' >> App-ordo.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="JSON::" />' >> App-ordo.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Mojo::UserAgent" />' >> App-ordo.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="POSIX::" />' >> App-ordo.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Term::ANSIColor" />' >> App-ordo.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Term::ReadKey" />' >> App-ordo.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Term::ReadLine::Perl5" />' >> App-ordo.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Text::Table::Tiny" VERSION="1.02" />' >> App-ordo.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Time::Local" />' >> App-ordo.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="feature::" />' >> App-ordo.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="strict::" />' >> App-ordo.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="warnings::" />' >> App-ordo.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="x86_64-linux-gnu-thread-multi-5.38" />' >> App-ordo.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> App-ordo.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> App-ordo.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> App-ordo.ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  'lib/App/ordo.pm' 'blib/lib/App/ordo.pm' 
...
config::
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)'\'')' -- \
	  'share/ordo_config.json' '$(INST_LIB)/auto/share/dist/$(DISTNAME)/ordo_config.json' 
